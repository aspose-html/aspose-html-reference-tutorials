...
- **Quelle version de Java est nécessaire ?** JDK8 ou supérieure.

## Qu'est-ce que « lire le fichier zip java » ?
...
- **Simplicité :** Configuration en une ligne (`ProtocolMessageFilter("zip")`) indique au moteur de routeur les requêtes ZIP vers votre code.
...
## Comment lire un fichier zip en Java – Étape 1 : Initialiser le gestionnaire
...
## Étape 2 : Implémenter la méthode Dispose (définir le type MIME en Java – libération des ressources)
...
## Étape 3 : Gérer les requêtes réseau – Cœur du processus de lecture de fichiers zip
...
## Étape 4 : Définir le type MIME en Java
...
## Étape 5 : Appeler le gestionnaire suivant – Fin du pipeline
...
|--------------|--------|--------------|
| `FileNotFoundException` | Le chemin à l’intérieur du ZIP est incorrect ou la barre oblique initiale est manquante. | Utilisez `context.getRequest().getRequestUri().getPathname().replaceFirst("^/", "")`. |
| Mauvais type de contenu | Le mappage MIME n’est pas reconnu pour les extensions obscures. | Ajoutez une mappage personnalisée avec `MimeType.registerExtension(".xyz", "application/xyz")`. |
| Pression mémoire sur les fichiers volumineux | `Files.readAllBytes` charge le fichier entier en mémoire. | Diffusez l’entrée en utilisant `InputStream` et le constructeur `ByteArrayContent` qui accepte un flux. |
