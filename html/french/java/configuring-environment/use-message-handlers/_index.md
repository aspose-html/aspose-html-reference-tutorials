...
- **Quelle version de Java est prise en charge ?** Tout JDK 8+ (le tutoriel utilise JDK 11).  
...
## Prérequis
Avant de plonger dans le guide étape par étape, assurons‑nous que vous avez tout ce qu’il faut :
...
2. Aspose.HTML for Java : Vous devez disposer d’Aspose.HTML for Java installé. Vous pouvez le télécharger depuis la [page des releases Aspose](https://releases.aspose.com/html/java/).
3. IDE : Utilisez votre environnement de développement Java préféré (IDE) comme IntelliJ IDEA, Eclipse ou NetBeans.
4. Connaissances de base en Java : Une familiarité avec la programmation Java est essentielle pour suivre ce tutoriel efficacement.
...
- **Erreurs de chemin de fichier** – Utilisez des chemins absolus ou définissez correctement le répertoire de travail lors du chargement de `document.html`.
