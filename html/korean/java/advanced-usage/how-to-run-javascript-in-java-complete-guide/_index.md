...
## 빠른 답변
- **Java에서 JavaScript를 실행할 수 있는 라이브러리는 무엇입니까?** Aspose.HTML의 내장 `ScriptEngine`.
- **브라우저를 설치해야 하나요?** 아니요, 엔진은 완전 헤드리스입니다.
- **기존 HTML 파일을 로드할 수 있나요?** 예 – 파일이나 URI를 수신하여 `HTMLDocument` 생성자를 사용하세요.
- **엔진은 스레드로부터 안전한가요?** 스레드당 별도의 `ScriptEngine`을 제작하거나 풀을 사용하세요.
- **어떤 Java 버전이 필요합니까?** Java8 이상 (예제는 Java11 사용).

## Java에서 “자바스크립트를 실행하는 방법”란?

Java 프로세스 내에서 JavaScript를 실행한다는 것은 제어할 수 있는 DOM과 블록 작동할 수 있는 JavaScript를 사용하는 것을 의미합니다. Aspose.HTML은 브라우저의 JavaScript 엔진과 동일하게 작동하지만 UI나 외장 헤드가 전혀 없는 'ScriptEngine'을 제공합니다.

## Java에서 JavaScript를 실행하는 이유는 무엇입니까?

- **서버 측 템플릿:** 클라이언트에 앞서 HTML을 동적으로 조정합니다.
- **자동화:** 클라이언트 측 연구소가 필요한 이메일, PDF 등을 생성합니다.
- **테스트:** 전체 브라우저 없이 CI 파이프라인에서 JavaScript 스니펫을 검증합니다.
...
- Aspose.HTML을 사용하여 **HTML 문서 Java**를 만드는 방법.
- 문서를 이해하는 **JavaScript 엔진**을 얻는 방법.
- 펼쳐서 Java를 사용하는 방법(예: 로거)을 참조하는 방법.
- DOM을 처리하기 위해 **Java에서 JavaScript를 실행**하는 방법.
- 펼쳐보기 **외부 HTML Java 가져오기**를 얻는 방법.
- 일반적인 레이아웃과 조언 준비.

## 1단계: HTML 문서 Java 스타일 만들기
...
## 4단계: DOM을 수정하는 JavaScript 코드 작성
...
## 6단계: 결과 HTML 가져오기 - 외부 HTML 가져오기 (Java 코드)
...
### 복잡한 Java 객체를 스크립트에 전달하려면 어떻게 해야 하나요?
...
- **엔진을 아껴서 재사용하세요:** 매 요청마다 새 엔진을 만들면 비용이 많이 들 수 있습니다. 처리량이 많다면 풀을 즐기세요.
- **입력 삭제:** 사용자가 파일을 허용하도록 허용하는 경우 샌드박스 처리 또는 API 범위를 제한해 보안 위험을 방지하세요.
- **메모리 관리:** 큰 DOM 트리는 드물게 힙을 찾을 수 있습니다. 사용이 가능하도록 `HTMLDocument`를 사용하세요(`htmlDoc.dispose()`가 제공된다면).

## 자주 묻는 질문

**Q: 킹 헤드리스 Linux 서버에서 싸울 수 있습니까?**
A: 네. Aspose.HTML `ScriptEngine`은 완전한 헤드리스이며 GUI 강력성이 없습니다.

**Q: Java17과 같은 최신 Java 버전에서도 작동합니까?**
A: 물론입니다. 이 라이브러리는 Java8+를 목표로 이에 따라 Java11, 17 및 이후 버전을 모두 지원합니다.

**Q: 메모리 없이 큰 HTML 파일을 처리하려면 어떻게 해야 합니까?**
A: 가능하다면 파일을 청크로 로드하거나 JVM 힙(`-Xmx`)을 가지고 사용 후 문서를 떠나는 것을 고려하세요.

**Q: 지휘권이 필요한가요?**
A: 네, 클러스터에는 Aspose.HTML이 필요합니다. 평가용 무료 체험판을 제공하고 있습니다.

**Q: 수정된 HTML에서 PDF를 생성하는 데 이 방식을 사용할 수 있습니까?**
A: 네. HTML을 최종적으로 수행하면 Aspose.HTML의 PDF 변환 API에 표시됩니다.
